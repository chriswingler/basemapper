...
By default, both markdown and raw text versions are generated. Use --md to generate only the markdown version.
...
def map_directory(root_dir: str, output_file: str, bmignore_path: Optional[str] = None, output_mode: str = "raw") -> None:
    """Map the directory structure and file contents.
    
    Args:
        root_dir: The directory to map
        output_file: The output file path
        bmignore_path: Optional path to a specific .bmignore file
        output_mode: Output mode - "raw", "markdown", or "both"
    """
    ...
    if output_mode in ["markdown", "both"]:
        print(f"Markdown version saved to: {output_file}")
...
def main() -> None:
    ...
    # Default to raw output, use --md for markdown only
    output_mode = "raw"
    args = sys.argv[:]
    if "--md" in args:
    ...
    if len(args) > 2:
        ...
        # Default output file in CWD - use .txt for raw mode
        if output_mode == "raw":
            output_file = os.path.join(os.getcwd(), "codebase_map.txt")
        else:
    ...
    print(f"Directory to map: {directory}")
    print(f"Output mode: {'Raw text (.txt)' if output_mode == 'raw' else 'Markdown (.md)'}")
    print(f"Output file: {output_file}")
    
    # Show help about options
    if output_mode == "raw":
        print("Tip: Use --md flag to generate markdown instead")
    ...
    map_directory(directory, output_file, bmignore_path, output_mode)
